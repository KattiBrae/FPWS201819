def vollesspektrum(x, y):
    ...
    plt.savefig('barium_vollesspektrum.pdf')
...
#    ax.fill_between(x, y-dy, y+dy, linewidth=3, color='C2', alpha=0.4, label='Unsicherheit')
    ...
    plt.savefig('barium_vollesspektrumlog.pdf')
...
#    ax.set_xlim(-200, 400)
    ...
    plt.savefig('barium_peak_%s.pdf' %peakno)
...
#    print('Inhalt Kontinuum!: ' + str(total))
    ...
    plt.savefig('barium_kontinuum.pdf')
...
#        print(theo/(const.elementary_charge*1e03))
        ...
        plt.savefig('barium_rueckstreupeak.pdf')

def spektrum_mit_annotations(x, y, peaks_x, peaksenergie, peakno, peaks_counts):
    ...
    ax.plot(E, np.zeros(len(E),)-40, 'x', color='C2', markersize=8, markeredgewidth=2, label='Literaturwerte')

    for i in range(len(peaks_counts)):
        if (i == 0):
            plt.annotate((xy)=(E[i]-80, 12+peaks_counts[i]), s='%.1f keV' %E[i], rotation=0)
        elif (i == 4):
            plt.annotate((xy)=(E[i]+7, 12+peaks_counts[i]), s='%.1f keV' %E[i], rotation=0)
        else:
            plt.annotate((xy)=(E[i]-87, 12+peaks_counts[i]), s='%.1f keV' %E[i], rotation=0)


    plt.grid(alpha=0.3)
    ax.legend(fancybox=True, ncol=1)
    ax.set_xlabel('Energie', labelpad=2)
    ax.set_ylabel('Häufigkeit', labelpad=8)

    plt.savefig('barium_markiert.pdf')
...
if __name__=="__main__":
    ...
    y = np.loadtxt('barium.txt', unpack=True,delimiter=' ')
...
#    yachse = ufloat(-1.22364356, 1.66905256e-01)
    ...
    peaks = sig.find_peaks(y, prominence = 30)
#    print(peaks)
    peaks = [397,  1339, 1466, 1724, 1857]

    peaksenergie = channelarraytoenergyarray(peaks, steigung, yachse)
#    print(peaksenergie)

#    peaks_x = [ 3197 ]
    peaks_x = [ 11.211850840000002, 37.326389080000006, 53.078015320000006, 81.05787772000001, 160.85230012, 276.29513979999996, 302.61693628, 356.0895622, 383.65490812 ]
...
#    vollesspektrum(x, y)
    spektrum_mit_annotations(x, y, peaks_x, peaksenergie, peakno, peaks_counts)
#    vollesspektrumlog(x, y)

#    gaussfit(channelkomplett, y, [397,  1339, 1466, 1724, 1857], 0, 8, 0, 15, 0, 0, steigung, yachse)
#    gaussfit(channelkomplett, y, [397,  1339, 1466, 1724, 1857], 1, 9, 0, 0, 0, 0, steigung, yachse)
#    gaussfit(channelkomplett, y, [397,  1339, 1466, 1724, 1857], 2, 9, 0, 0, 0, 0, steigung, yachse)
#    gaussfit(channelkomplett, y, [397,  1339, 1466, 1724, 1857], 3, 10, 0, 0, 0, -3, steigung, yachse)
#    gaussfit(channelkomplett, y, [397,  1339, 1466, 1724, 1857], 4, 9, 0, 0, 0, 0, steigung, yachse)
