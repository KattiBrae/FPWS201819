...
\input{data/tables/Tex/creichweite.tex}
\input{data/tables/Tex/reichweite_ohnefolie2.tex}
...
\begin{equation}
    n =  \frac{\rho}{M} \cdot N_A \approx \SI{5,9 \cdot 10^{28}}{\meter}^{-2}
\end{equation}
\begin{center}
...
Da die $\alpha$-Teilchen durch eine Schlitzblende mit $r = \SI{2}{\milli \meter} $ fliegt und bis zur Goldfolie eine Strecke von $R = \SI{4,1}{\centi \meter}$ zurücklegt, gilt für den Raumwinkel:
\begin{equation}
    \Omega = \frac{\pi r^2}{R^2} = \pi \cdot \left( \frac{2}{41} \right)^2 .
\end{equation}

Mit der bereits bestimmten Aktivität $\SI{15.64(48)}{\kilo\becquerel}$ pro Raumwinkelelementelement und einer Dicke von $\SI{2}{\micro \meter}$ ergeben sich die Werte in Tabelle \ref{tab:diffWQ}, wobei sich die theoretischen Wert aus Gleichung \eqref{eqn:rutherford} ergben. \\
Die gemessenen und theoretischen Werte sind zusätzlich in Abbildung \ref{fig:ruther} gegenübergestellt.
\input{data/tables/Tex/diffWQ.tex}
...
\input{data/tables/Tex/fordnungszahl.tex}
\begin{figure}
    \centering
