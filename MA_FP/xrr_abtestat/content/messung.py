if __name__=="__main__":
    ...
    import cmath as cm  # was sich halt so ansammelt und was man hier und da gebrauchen könnte...

### Variablen für die ganze Datei
...
#########################################################################
    ...
    x, y = np.loadtxt('1416_messung.txt', unpack=True,delimiter=' , ') # Einlesen der Messdaten
...
### Verwendete alternative Linestyles
    ...
    fig, ax = plt.subplots()
### ab hier werden die Daten aufbereitet
    ...
    counts_korrigiert_diffus = korrektur_diffuse_messung(counts, b) # Korrigieren der Messung um die diffuse Messung
    counts_korrigiert_geometriefaktor = korrektur_geometriefaktor() # Korrigieren der korrigierten Messung um den Geometriefaktor
    counts_final = counts_korrigiert_geometriefaktor    # Umbennenung da mein Variablenname furchtbar war...
    minimum_x, minimum_y, peak_array = finde_minima(counts_final, q) # Findet Minima im logaritmischen Graph und gibt verschiedene Arrays mit xy-Position und zugehörigem Array-Index aus
    schichtdicke = berechne_schichtdicke(q, peak_array) # Selbsterklärend
...
### Hier wird geplottet
    ax.plot(q[beg:end], unkorr,
        linestyle=denslydotted,
        linewidth = 1,
        color='C2',
        label='Unkorrigierte Daten ' +r'$\times 0.1$',
        alpha=0.8)
    ax.plot(a[beg:end], diffusedaten,
        linestyle='-',
        linewidth = 1,
        color='C5',
        label='Diffuse Messung ' +r'$\times 0.1$',
        alpha=0.8)
    ax.plot(a[beg:end], counts_korrigiert_diffus[beg:end],
    ...
    ax.plot(q[beg:end], counts_final[beg:end],
        linestyle='-',
        color='C0',
        label='Daten um Geometriefaktor korrigiert')
    ax.plot(minimum_x, minimum_y,
        markerstyle='+',
        color='C3',
        markersize=10,
        markeredgewidth=1,
        label='Verw. Minima für Schichtdicke: \n' + r'$\rightarrow$' +
            r'z = (%.2f' %(schichtdicke.n * 1e9) + '$\pm$' +
            r'%.2f)' %(schichtdicke.s * 1e9) + r'$\cdot 10^{-10}$ m')

    alphakritPS, qkritPS = kritischerwinkel(delta1) # berechnet den kritischen Winkel und plottet ihn anschließend als vertikale Linie
    ax.axvline(qkritPS, ymin=0, ymax=1,
        linewidth=0.7,
        linestyle=longdashs,
        color='C8',
        label=r'$\alpha_c (PS)=%.3f °$' %alphakritPS)
    ...
    ax.axvline(qkritSi, ymin=0, ymax=1,
        ...
        linestyle=longdashs,
        color='C9',
        label=r'$\alpha_c (Si)=%.3f °$' %alphakritSi)
#########################################################################
...
### hier werden die Ergebnisse auch in die Box im Plot geplottet
    textstr = '\n'.join((
        ...
        r'$n_2      = $'+'\t'+
        r'$ 1 - %.1f \cdot 10^{-6} $' % (delta1*1e06) + '  (PS)',

        r'$n_3      = $'+'\t'+
        r'$ 1 - %.1f \cdot 10^{-6} $' % (delta2*1e06) + '  (Si)',

        r'$\sigma_1 = $'+'\t'+
        r'$ %.1f \cdot 10^{-10} m $' % (sigma1*1e10) + '  (Luft/PS)',

        r'$\sigma_2 = $'+'\t'+
        r'$ %.1f \cdot 10^{-10} m $' % (sigma2*1e10) + '  (PS/Si)',

        '  ' + r'$z        = $'+'\t  '+
        r'$  %s \cdot 10^{-10} m$' % (int(z*1e10)) + '  (PS)',))

    props = dict(facecolor='white',
        edgecolor=(0.808, 0.808, 0.808),
        alpha=0.8,
...
### Plot-Basics
    ...
    ax.set_xlabel('q in ' +r'$\frac{1}{m}$',
        labelpad=2)
    ax.set_ylabel('Reflektivität',
        labelpad=8)
    fig.tight_layout()
...
def parratt(z):
    ...
    x1=(r12+x2)/(1+r12*x2)
    par = np.abs(x1)**2
    # Es gibt eine ganze Strecke komplexe Daten vor Beginn der Kiessing-Oszillationen, daher werden die ersten Werte des Arrays nachträglich auf 1 gesetzt. Sieht interessant aus, wenn man einen Betrag um den Inhalt der Wurzel in den modifizierten Fresnel-Koeffizienten setzt. Ich hab den Runtime Error ertragen...
...
### bin kein Informatik-Talent, aber ich bemühe mich um Übersichtlichkeit - ob das geklappt hat, werden wir sehen.
