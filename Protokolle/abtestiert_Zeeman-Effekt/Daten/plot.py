...
print('Graphik1')
p, q = np.loadtxt('hysteresewerte.txt', unpack=True, delimiter='&')
...
x_new = np.linspace(x[0], x[16], 500)
...
#c, v = np.loadtxt('hystereseabsteigend.txt', unpack=True,delimiter='&')
#def f(c,n,m):
#    return n*c+m
...
plt.figure(1)
plt.plot(p,q,'x', label='Messwerte')
